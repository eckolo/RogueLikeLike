 ...
 * 
 * // This same as Assert.AreEqual(Enumerable.Range(1,5).ToArray(), new[]{1, 2, 3, 4, 5}.ToArray())
 ...
 * new[] { "a", "z", "x" }.IsNot("a", "x", "z"); /// Assert.AreNotEqual
 *
...
namespace Assets.Editor.TEST.Domain.Service
{
    ...
    public static class ChainingAssertion
    {
        ...
        {
            Assert.AreNotEqual(notExpected.ToArray(), actual.ToArray());
        }
        ...
        {
            Assert.AreEqual(typeof(TExpected), value.GetType());
        ...
        {
            Assert.AreNotEqual(typeof(TWrong), value.GetType());
        }
        ...
        public static void IsNotStructuralEqual(this object actual, object expected, string message = "")
        {
            ...
            if(r.IsEquals)
            {
                throw new NUnitException("is structural equal" + message);
            }
        }
